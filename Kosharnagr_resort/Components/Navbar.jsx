  ...
  return (
    <>
      ...
      <div className="fixed top-0 left-0 w-full bg-white  p-7 flex justify-around items-center border-b border-gray-200 shadow-sm z-[100] md:gap-[75px] ">
        ...
        {/* Logo */}
        <div className="text-2xl text-black font-bold font-serif">{data.companyInfo.name}</div>
          ...
          <ul className="flex gap-10 text-lg items-center">
            {data.navigation.map((item, i) => (
              <li key={i} className="relative group cursor-pointer text-black">
                <span className="transition duration-200 group-hover:text-slate-400">{item}</span>
      ...
      >
        <ul className="flex flex-col gap-4 px-6 py-6 text-lg font-medium">
          {data.navigation.map((item, i) => (
            <li
           key={i}
               className="group cursor-pointer text-black border border-gray-400 rounded-xl w-full px-4 py-3 bg-gray-50 hover:bg-gray-100 transition-all relative overflow-hidden"
         >
            <span className="transition duration-200 group-hover:text-slate-500 z-10 relative">{item}</span>
